...
def cond_1():
   ...
   '''
    ...
    interval1 = 5 * 60  # 3 minutes * 60 seconds/minutes for emotion assessment
    ...
    if object_current == object_correct and object_current != '':  # the current object detected on the readert you can add an action if the object was changed
        ...
        if current_time > experiment_end_time:
            ...
            np.save('placements', constants.memory_performance)
...
def personalized_hint(placement_memory, correct_placement, associative_memory, object_placed, prep_location):
    ...
    if current_place in placement_memory:
    ...
    elif associative_memory[location] > 1 and associative_memory[preposition1] > 1:
        ...
        print(associative_memory)
        print('LOCATION')
        hintn = random.randrange(4)  # random integer from 0 to 3 I have not yet implemented the 3rd hint, see comment
...
if __name__ == "__main__":
    ...
    last_mtime = time.time()
    experiment_end_time = experiment_start_time + 5 * 60  # 30 minutes * 60 seconds/min
    emotion_no = 0
    ...
    for i in range(1, num_ports):
        ...
        reader_ports['reader{}_port'.format(i)] = serial.Serial(port_name, 9600, timeout=5)
        #print(port_name)
