...
'''This code needs a connection to Pepper robot. Currently the below code works with three readers. Can be used for conditions 1 to 3 (uncomment or comment which you need)
'''
    ...
    if tag is not None and tag != '' and not np.isin(reader, readers_correct):
        ...
        if reader == 'reader0':
            ...
            if not object_correct == object_current:
                ...
                animated_speech.say('Bitte stelle ein neues Objekt immer zuerst auf die Start Box, damit ich dir sagen kann wo es hingehoert.')
        ...
        if tag is not None and tag != '' and not np.isin(reader, readers_correct):
            ...
            if reader == 'reader0':
                ...
                if not object_correct == object_current:
                    ...
                    location_correct = ''
                    speech_service.setParameter("speed", speech_rate_base)
                    animated_speech.say('Bitte stelle ein neues Objekt immer zuerst auf die Start Box, damit ich dir sagen kann wo es hingehoert.')  # the object
...
def cond_1():
   ...
   '''
    ...
    if object_current == object_correct and object_current != '':  # the current object detected on the readert you can add an action if the object was changed
        ...
        # add placement to placement memory
        ...
        if object_location_corr == object_location:
            ...
            # COND 4: add hint from table 1 X% of the time
            print('The emotion number is: ', emotion_no)
            if emotion_no >= 1:
                ...
                if x_perf / placement_no <= 0.5:  # RED side of behaviors
                    ...
                    if joy > 3:  # BEHAVIOR 4: hints after 60% of placements
                        ...
                        if i <= 60:  # in 60 % of placements
                            ...
                            personalized_hint(constants.memory_placements, object_location_corr,constants.memory_association, object_current, location_current)
                                # print(hint)
                    ...
                    if joy >= 3:  # BEHAVIOR 1: no hints
                        ...
                        if i <= 40:  # hint in 40 % of placements
                            ...
                            if behavior == 1 or behavior == 2:  # motivational message
                                hintn = random.randrange(16)  # random integer from 0 to 15
                                hint = robot_text.robot_con30[hintn]
                                speech_service.setParameter("speed", speech_rate_base)
                                robot_speech([hint])
...
def check_incorrect_placements():
    ...
    '''This Function checks if an object has been left on a reader incorrectly.'''
    ...
    for port_name, port in reader_ports.items():
        ...
        tag2, reader2 = extract_identifiers(data2)
        print('Incorrect placement', tag2, reader2)
        # print "Data from {}: {}".format(port_name, data)

        if tag2 is not None and tag2 != '' and not np.isin(reader2, readers_correct):
            tag_dict = {'object': tag2, 'location': reader2}
            location_check = ''
            location_correctv2 = '1'
            if not reader2 == 'reader0':
                with lock:
                    object_check, location_check, location_correctv2 = retrieve_tag(
                        tag_dict)  # the object the robot identifies to be placed
                if not location_check == location_correctv2:
                    time.sleep(0.2)
                    speech_service.setParameter("speed", speech_rate_base)
                    animated_speech.say('Du hast ein Objekt an einer falschen Stellen stehengelassen.')
...
def personalized_hint(placement_memory, correct_placement, associative_memory, object_placed, prep_location):

    global hint_counter
    global hint_memory
    global placement_no
    
    parts = prep_location.rsplit(' ', 1)
    preposition = parts[0]
    prepositions = preposition.split()
    preposition1 = prepositions[0]

    location = parts[1]
    current_place = object_current + prep_location
    print(preposition1, location, associative_memory)

    if current_place in placement_memory:
        print(placement_memory)
        print('PLACEMENT PREVIOUSLY DONE')
        hintn = random.randrange(5)  # random integer from 0 to 4
        hint = robot_text.robot_con70[hintn]
        ...
        # add given hint and current placement number to memory
    ...
    elif associative_memory[location] > 3 and associative_memory[preposition1] > 3:
        print(associative_memory)
        print('LOCATION AND PREPOSITION')
        hintn = random.randrange(5)  # random integer from 0 to 4
        hint = robot_text.robot_con60[hintn]
        while hint == hint_memory[-1]:  # avoid same hint being used twice
            hintn = random.randrange(5)
            hint = robot_text.robot_con60[hintn]
    ...
    elif associative_memory[location] > 1:
        print(associative_memory)
        print('LOCATION')
        prep_cur = location_current.split(" ", -1)
        prep_cor = location_correct.split(" ", -1)
        speech_service.setParameter("speed", speech_rate_base)
        if prep_cor == prep_cur:
            if hint_counter > 0:
                hint = robot_text.robot_con40[4]
                robot_speech2([hint], preposition)
                hint_counter += -1
        else:
            hintn = random.randrange(5)
    ...
    elif associative_memory[preposition1] > 1:
        print(associative_memory)
        print('PREPOSITION')
        loc_cur = location_current.split(" ", -1)
        loc_cor = location_correct.split(" ", -1)
        speech_service.setParameter("speed", speech_rate_base)
        if loc_cor == loc_cur:
            if hint_counter > 0:
                hint = robot_text.robot_con50[4]
                robot_speech2([hint], location_current)
                hint_counter += -1
        else:
            hintn = random.randrange(5)
            hint = robot_text.robot_con50[hintn]
            while hint == hint_memory[-1]:  # avoid same hint being used twice
                hintn = random.randrange(5)
                hint = robot_text.robot_con50[hintn]
